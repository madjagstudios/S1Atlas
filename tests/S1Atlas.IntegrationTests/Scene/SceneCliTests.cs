...
public sealed class SceneCliTests : IAsyncDisposable
{
    ...
    public async Task Scriptable_object_command_prints_the_asset_and_its_fields_in_human_and_json()
    {
        ...
        Assert.Contains("Field: Leader.Data (PPtr<$Widget>) = 0:12 -> Component \"Root\" (Game.Widget) | id component-a", humanOutput.ToString(), StringComparison.Ordinal);
        Assert.Contains("Field: Backup (PPtr<$Widget>) = 0:0 -> null", humanOutput.ToString(), StringComparison.Ordinal);
        Assert.Contains("Field: Effects.Array[0] (PPtr<$Effect>) = 9:5 -> unresolved (fileId=9;localFileId=5;external=<missing-table-entry>)", humanOutput.ToString(), StringComparison.Ordinal);
        ...
        var target = json.RootElement.GetProperty("data").GetProperty("scriptFields").GetProperty("fields")[1].GetProperty("target");
        Assert.Equal("Root", target.GetProperty("name").GetString());
        Assert.Equal("component-a", target.GetProperty("id").GetString());
    }
    ...
    public async Task Component_command_prints_decoded_fields_in_human_and_json()
    {
        ...
        var humanExit = application.Invoke(["component", "component-a"], humanOutput, humanError, TestContext.Current.CancellationToken);
        var jsonExit = application.Invoke(["component", "component-a", "--json"], jsonOutput, jsonError, TestContext.Current.CancellationToken);

        Assert.True(humanExit == 0, humanOutput.ToString() + humanError); Assert.True(jsonExit == 0, jsonOutput.ToString() + jsonError);
        Assert.Contains("Field: Price (float) = 50000", humanOutput.ToString(), StringComparison.Ordinal);
        using var json = System.Text.Json.JsonDocument.Parse(jsonOutput.ToString());
        var field = json.RootElement.GetProperty("data").GetProperty("scriptFields").GetProperty("fields")[0];
        Assert.Equal("50000", field.GetProperty("value").GetString());
    }
