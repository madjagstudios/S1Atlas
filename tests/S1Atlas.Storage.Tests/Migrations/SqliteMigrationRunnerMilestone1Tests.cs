...
public sealed class SqliteMigrationRunnerMilestone1Tests : IAsyncDisposable
{
    ...
    [Fact]
    public void MigrationsThroughFourteen_HaveCommittedNamesAndChecksums_AndEarlierMigrationsRemainPinned()
    {
        ...
        Assert.Equal(13, SqliteMigrations.All[12].Version);
        Assert.Equal("scene-type-tree-source-v13", SqliteMigrations.All[12].Name);
        Assert.Equal(14, SqliteMigrations.All[13].Version);
    ...
    [Fact]
    public async Task VersionEightDatabase_MigratesToFourteen_WithCallableSurfaceVisibilityReferenceTablesAndRelationshipQueryIndex()
    {
        ...
        Assert.Equal(14L, await ScalarAsync(connection, "SELECT MAX(version) FROM schema_migrations;", cancellationToken));
        ...
        Assert.Equal(0L, await ScalarAsync(
            connection,
            "SELECT COUNT(*) FROM pragma_table_info('symbols') WHERE name = 'melon_loader_path';",
            cancellationToken));
        Assert.Equal(1L, await ScalarAsync(
            connection,
            "SELECT COUNT(*) FROM pragma_table_info('symbols') WHERE name = 'is_public';",
            cancellationToken));
        Assert.Equal(1L, await ScalarAsync(
            connection,
        ...
        await ExecuteAsync(connection, """
            ...
            VALUES ('snapshot-v9', 'ScheduleI', 'Installed', 'extraction-v9', '2026-08-27T00:00:00Z');
            INSERT INTO symbols(symbol_id, snapshot_id, canonical_key, kind, qualified_name, signature, is_best_effort)
            VALUES ('symbol-v9', 'snapshot-v9', 'ScheduleI:Installed:Method:Demo::Run()', 'Method', 'Demo.Run', 'Demo::Run()', 0);
            """, cancellationToken);
        Assert.Equal(0L, await ScalarAsync(connection, "SELECT is_public FROM symbols WHERE symbol_id = 'symbol-v9';", cancellationToken));
    }
    ...
    public async Task VersionSevenDatabase_MigratesToThirteenExactlyOnce_AndTheSecondRunIsIdempotent()
    {
        ...
        Assert.Equal(
            ...
            await TextScalarAsync(
                ...
                cancellationToken));
        Assert.Equal(
            SqliteMigrations.All[10].Checksum,
            await TextScalarAsync(
                connection,
        ...
        Assert.Equal(7L, await ScalarAsync(
            ...
            cancellationToken));
        Assert.Single(Directory.GetFiles(_backupDirectory, "atlas-before-schema-14-*.db", SearchOption.TopDirectoryOnly));
    }

    [Fact]
    public async Task VersionSixDatabase_MigratesToFourteen_PreservesSymbolsAndAddsNullableBodyRecoveryStatus()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        ...
        Assert.Equal(14L, await ScalarAsync(migrated, "SELECT MAX(version) FROM schema_migrations;", cancellationToken));
        ...
        Assert.Equal(1L, await ScalarAsync(migrated, "SELECT COUNT(*) FROM sqlite_schema WHERE type = 'index' AND name = 'ix_relationships_snapshot_kind_target_text';", cancellationToken));
