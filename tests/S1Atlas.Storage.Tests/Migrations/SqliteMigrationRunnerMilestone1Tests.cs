...
public sealed class SqliteMigrationRunnerMilestone1Tests : IAsyncDisposable
{
    ...
    [Fact]
    public void MigrationsThroughThirteen_HaveCommittedNamesAndChecksums_AndEarlierMigrationsRemainPinned()
    {
        Assert.Equal(13, SqliteMigrations.All.Count);
    ...
    [Fact]
    public async Task VersionEightDatabase_MigratesToThirteen_WithCallableSurfaceVisibilityReferenceTablesAndRelationshipQueryIndex()
    {
        ...
        Assert.Equal(13L, await ScalarAsync(connection, "SELECT MAX(version) FROM schema_migrations;", cancellationToken));
    ...
    public async Task VersionSevenDatabase_MigratesToThirteenExactlyOnce_AndTheSecondRunIsIdempotent()
    {
        ...
        Assert.Equal(7L, await ScalarAsync(
            ...
            cancellationToken));
        Assert.Single(Directory.GetFiles(_backupDirectory, "atlas-before-schema-13-*.db", SearchOption.TopDirectoryOnly));
    }

    [Fact]
    public async Task VersionSixDatabase_MigratesToThirteen_PreservesSymbolsAndAddsNullableBodyRecoveryStatus()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        ...
        Assert.Equal(13L, await ScalarAsync(migrated, "SELECT MAX(version) FROM schema_migrations;", cancellationToken));
