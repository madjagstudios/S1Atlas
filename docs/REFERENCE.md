...
## Managed Unity class database pin

The committed definition `config/tools/unity-classdata.win-x64.json` pins the
AssetsTools.NET class package that the scene parser uses when a SerializedFile
container strips its type tree (every Schedule I release container does):

```text
Tool ID:       unity-classdata
Version:       uabea-5adb448
Source:        nesrak1/UABEA, commit 5adb448deeefa1b88881f1fa44243009b352db3a, ReleaseFiles/classdata.tpk
Expected size: 289,605 bytes
SHA-256:       129e1f80f930415db6779fe6089afa75280cb51462bcee812beab6cd81a764c6
Local name:    classdata.tpk
Licence:       MIT (UABEA repository)
Probes:        none (data-only package)
```

`tools install unity-classdata` verifies the exact size and SHA-256 before the
file is stored; it is never executed. The scene parser re-hashes the installed
file against the pin before interpreting it, resolves the class layouts for the
container's Unity version, and records the source, version, hash, and the
resolved dump version in the snapshot's `typeTreeSource` provenance (the newest
2022.3 dump in this package is `2022.3.26f1`, which stands in for
`2022.3.62f2`). The descriptor is part of the scene snapshot identity. See
[docs/dependencies/unity-classdata-uabea-5adb448.md](dependencies/unity-classdata-uabea-5adb448.md)
for the licence review and the update procedure. Automated tests use synthetic
in-memory class databases and generated bytes; they never download the package.
