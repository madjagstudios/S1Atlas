...
internal sealed record ComponentOutput(SceneQueryStatus Status, SceneSnapshotRecord? Snapshot, SceneComponentRecord? Component, IReadOnlyList<SceneComponentRecord> Candidates, ScenePageResult<SceneReferenceRecord> References, SymbolQueryResult? CodeSymbol, IReadOnlyList<SceneContainerRecord>? Containers = null, SceneScriptFieldSetRecord? ScriptFields = null);
internal sealed record SceneIndexOutput(string SceneSnapshotId, string BuildId, string CodeIndexId, string ParserId, string ParserVersion, bool Reused, int ContainerCount, int DocumentCount, int GameObjectCount, int TransformCount, int ComponentCount, int ReferenceCount, IReadOnlyDictionary<string, int> RecoveryCounts, IReadOnlyList<string> Warnings, string? TypeTreeSource = null, string? ScriptLayoutSource = null);
internal sealed record ScriptableAssetOutput(SceneQueryStatus Status, SceneSnapshotRecord? Snapshot, SceneScriptableAssetRecord? Asset, IReadOnlyList<SceneScriptableAssetRecord> Candidates, SceneScriptFieldSetRecord? ScriptFields, IReadOnlyList<SceneContainerRecord>? Containers = null);
