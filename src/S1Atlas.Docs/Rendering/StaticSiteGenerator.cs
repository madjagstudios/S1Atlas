...
public sealed class StaticSiteGenerator
{
    ...
    public async Task GenerateAsync(PortalSiteModel site, string outputDirectory, CancellationToken cancellationToken = default)
    {
        ...
        {
            ["index.html"] = _pages.Render("index.html", "S1Atlas.Docs", Landing(site, allSymbols, "index.html")),
        ...
        {
            var indexSymbols = index.Namespaces.SelectMany(namespaceModel => namespaceModel.Symbols).ToArray();
            ...
            foreach (var symbol in indexSymbols.Where(symbol => IsStandalone(symbol.Kind)))
            {
                var body = _sections.Provenance(index) + _sections.Symbol(index, symbol, symbol.PagePath);
                if (symbol.Kind == SymbolKind.Type)
                    body += InlineMembers(indexSymbols, symbol);
        ...
        foreach (var asset in new StaticAssets().Render(allSymbols)) files[asset.Key] = asset.Value;
    ...
    private string Diff(PortalSiteModel site, PortalDiffModel diff)
    {
        var counts = string.Join(string.Empty, Enum.GetValues<DiffClassification>().Select(classification =>
            $"<li>FACT: {HtmlPageRenderer.Escape(classification.ToString())}: {_text.FormatCount(diff.Result.CountsByClassification.GetValueOrDefault(classification))}.</li>"));
    ...
    private string SymbolHistory(PortalSymbolHistoryModel history)
    {
        ...
        return $"<section id=\"evidence\"><h2>FACT evidence</h2><p>FACT: Schedule I cross-build history for <code>{HtmlPageRenderer.Escape(history.CanonicalKey)}</code>.</p><p>DERIVED: showing {_text.FormatCount(history.Occurrences.Count)} verified indexed occurrences.</p>{occurrences}</section>";
    }
